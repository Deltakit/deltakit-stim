...
Examples:
    ...
    add_gate(
        failed,
        Gate{
    .name = "HERALD_LEAKAGE_EVENT", .id = GateType::HERALD_LEAKAGE_EVENT,
    .best_candidate_inverse_id = GateType::HERALD_LEAKAGE_EVENT, .arg_count = ARG_COUNT_SYGIL_ZERO_OR_ONE,
    .flags = (GateFlags)(GATE_IS_SINGLE_QUBIT_GATE | GATE_PRODUCES_RESULTS | GATE_IS_NOISY |
                         GATE_ARGS_ARE_DISJOINT_PROBABILITIES),
    .category = "L_Collapsing Gates", .help = R"MARKDOWN(
