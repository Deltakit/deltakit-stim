...
def test_circuit_append_operation():
    ...
    with pytest.raises(ValueError, match="invalid modifiers"):
        c.append_operation("X", [deltakit_stim.target_inv(0)])
    with pytest.raises(ValueError, match="invalid modifiers"):
        c.append_operation("X", [deltakit_stim.target_x(0)])
    with pytest.raises(IndexError, match="lookback"):
        deltakit_stim.target_rec(0)
    with pytest.raises(IndexError, match="lookback"):
        deltakit_stim.target_rec(1)
    with pytest.raises(IndexError, match="lookback"):
        deltakit_stim.target_rec(-2**30)
    assert deltakit_stim.target_rec(-1) is not None
    assert deltakit_stim.target_rec(-15) is not None

    c.append_operation("X", [0])
    c.append_operation("X", [1, 2])
    c.append_operation("X", [3])
    c.append_operation("CNOT", [0, 1])
    c.append_operation("M", [0, deltakit_stim.target_inv(1)])
    c.append_operation("X_ERROR", [0], 0.25)
    c.append_operation("CORRELATED_ERROR", [deltakit_stim.target_x(0), deltakit_stim.target_y(1)], 0.5)
    c.append_operation("DETECTOR", [deltakit_stim.target_rec(-1)])
    c.append_operation("OBSERVABLE_INCLUDE", [deltakit_stim.target_rec(-1), deltakit_stim.target_rec(-2)], 5)
    assert str(c).strip() == """
X 0 1 2 3
CX 0 1
M 0 !1
...
def test_circuit_eq():
    ...
    b = """
        ...
        M 0
    """
    assert deltakit_stim.Circuit() == deltakit_stim.Circuit()
    assert deltakit_stim.Circuit() != deltakit_stim.Circuit(a)
    assert not (deltakit_stim.Circuit() != deltakit_stim.Circuit())
    assert not (deltakit_stim.Circuit() == deltakit_stim.Circuit(a))
    assert deltakit_stim.Circuit(a) == deltakit_stim.Circuit(a)
    assert deltakit_stim.Circuit(b) == deltakit_stim.Circuit(b)
    assert deltakit_stim.Circuit(a) != deltakit_stim.Circuit(b)

    assert deltakit_stim.Circuit() != None
    assert deltakit_stim.Circuit != object()
    assert deltakit_stim.Circuit != "another type"
    assert not (deltakit_stim.Circuit == None)
    assert not (deltakit_stim.Circuit == object())
    assert not (deltakit_stim.Circuit == "another type")
...
def test_hash():
    # deltakit_stim.Circuit is mutable. It must not also be value-hashable.
    # Defining __hash__ requires defining a FrozenCircuit variant instead.
    with pytest.raises(TypeError, match="unhashable"):
        _ = hash(deltakit_stim.Circuit())


def test_circuit_generation():
    surface_code_circuit = deltakit_stim.Circuit.generated(
...
def test_num_detectors():
    assert deltakit_stim.Circuit().num_detectors == 0
    assert deltakit_stim.Circuit("DETECTOR").num_detectors == 1
    assert deltakit_stim.Circuit("""
        REPEAT 1000 {
            DETECTOR
        }
...
def test_num_observables():
    assert deltakit_stim.Circuit().num_observables == 0
    assert deltakit_stim.Circuit("OBSERVABLE_INCLUDE(0)").num_observables == 1
    assert deltakit_stim.Circuit("OBSERVABLE_INCLUDE(1)").num_observables == 2
    assert deltakit_stim.Circuit("""
        M 0
...
def test_indexing_operations():
    ...
    c = deltakit_stim.Circuit('X 0')
    assert len(c) == 1
    assert list(c) == [deltakit_stim.CircuitInstruction('X', [deltakit_stim.GateTarget(0)])]
    assert c[0] == c[-1] == deltakit_stim.CircuitInstruction('X', [deltakit_stim.GateTarget(0)])
    with pytest.raises(IndexError):
        _ = c[1]
    with pytest.raises(IndexError):
        _ = c[-2]
    ...
    with pytest.raises(IndexError):
        _ = c[-4]
    assert list(c) == [
        deltakit_stim.CircuitInstruction('X', [deltakit_stim.GateTarget(5), deltakit_stim.GateTarget(6)]),
        deltakit_stim.CircuitRepeatBlock(1000, deltakit_stim.Circuit('H 5')),
        deltakit_stim.CircuitInstruction('M', [deltakit_stim.GateTarget(deltakit_stim.target_inv(0))]),
    ]
...
def test_slicing():
    ...
    assert c[:] is not c
    assert c[:] == c
    assert c[1:-1] == deltakit_stim.Circuit("""
        REPEAT 5 {
            X 1
        }
        Y 2
    """)
    assert c[::2] == deltakit_stim.Circuit("""
        H 0
        Y 2
    """)
    assert c[1::2] == deltakit_stim.Circuit("""
        REPEAT 5 {
            X 1
        }
        Z 3
...
def test_reappend_gate_targets():
    expected = deltakit_stim.Circuit("""
        MPP !X0 * X1
        CX rec[-1] 5
    """)
    c = deltakit_stim.Circuit()
    c.append_operation("MPP", cast(deltakit_stim.CircuitInstruction, expected[0]).targets_copy())
    c.append_operation("CX", cast(deltakit_stim.CircuitInstruction, expected[1]).targets_copy())
    assert c == expected
...
def test_append_instructions_and_blocks():
    ...
    assert c == deltakit_stim.Circuit("TICK")

    with pytest.raises(ValueError, match="no targets"):
        c.append_operation("TICK", [1, 2, 3])

    c.append_operation(deltakit_stim.Circuit("H 1")[0])
    assert c == deltakit_stim.Circuit("TICK\nH 1")

    c.append_operation(deltakit_stim.Circuit("CX 1 2 3 4")[0])
    assert c == deltakit_stim.Circuit("""
        TICK
        H 1
        CX 1 2 3 4
    ...
    c.append_operation((deltakit_stim.Circuit("X 5") * 100)[0])
    assert c == deltakit_stim.Circuit("""
        TICK
        H 1
        CX 1 2 3 4
    ...
    c.append_operation(deltakit_stim.Circuit("PAULI_CHANNEL_1(0.125, 0.25, 0.325) 4 5 6")[0])
    assert c == deltakit_stim.Circuit("""
        TICK
        H 1
        CX 1 2 3 4
    ...
    with pytest.raises(ValueError, match="targets"):
        c.append_operation(deltakit_stim.Circuit("H 1")[0], [2])

    with pytest.raises(ValueError, match="arg"):
        c.append_operation(deltakit_stim.Circuit("H 1")[0], [], 0.1)

    with pytest.raises(ValueError, match="targets"):
        c.append_operation((deltakit_stim.Circuit("H 1") * 5)[0], [2])

    with pytest.raises(ValueError, match="arg"):
        c.append_operation((deltakit_stim.Circuit("H 1") * 5)[0], [], 0.1)

    with pytest.raises(ValueError, match="repeat 0"):
        c.append_operation(deltakit_stim.CircuitRepeatBlock(0, deltakit_stim.Circuit("H 1")))
...
def test_approx_equals():
    base = deltakit_stim.Circuit("X_ERROR(0.099) 0")
    assert not base.approx_equals(deltakit_stim.Circuit("X_ERROR(0.101) 0"), atol=0)
    assert not base.approx_equals(deltakit_stim.Circuit("X_ERROR(0.101) 0"), atol=0.00001)
    assert base.approx_equals(deltakit_stim.Circuit("X_ERROR(0.101) 0"), atol=0.01)
    assert base.approx_equals(deltakit_stim.Circuit("X_ERROR(0.101) 0"), atol=999)
    assert not base.approx_equals(deltakit_stim.Circuit("DEPOLARIZE1(0.101) 0"), atol=999)
...
def test_backwards_compatibility_vs_safety_append_vs_append_operation():
    ...
    assert c == deltakit_stim.Circuit("X_ERROR(0) 5")
...
def test_blocked_remnant_edge_error():
    ...
    assert circuit.detector_error_model(decompose_errors=True) == deltakit_stim.DetectorErrorModel("""
        error(0.125) D2 D3
    ...
    assert circuit.detector_error_model(
        decompose_errors=True,
        block_decomposition_from_introducing_remnant_edges=True,
        ignore_decomposition_failures=True) == deltakit_stim.DetectorErrorModel("""
...
def test_shortest_graphlike_error():
    ...
    assert len(actual) == 1
    assert isinstance(actual[0], deltakit_stim.ExplainedError)
...
def test_without_noise():
    ...
    """).without_noise() == deltakit_stim.Circuit("""
        CNOT 0 1
        M 0
        REPEAT 50 {
            X 0 1 2
...
def test_flattened():
    ...
    """).flattened() == deltakit_stim.Circuit("""
...
def test_circuit_from_file():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(path, 'w') as f:
            print('H 5', file=f)
        assert deltakit_stim.Circuit.from_file(path) == deltakit_stim.Circuit('H 5')
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(path) as f:
            assert deltakit_stim.Circuit.from_file(f) == deltakit_stim.Circuit('CX 4 5')

    with pytest.raises(ValueError, match="how to read"):
        deltakit_stim.Circuit.from_file(object())
    with pytest.raises(ValueError, match="how to read"):
        deltakit_stim.Circuit.from_file(123)
...
def test_circuit_slice_reverse():
    c = deltakit_stim.Circuit()
    assert c[::-1] == deltakit_stim.Circuit()
    c = deltakit_stim.Circuit("X 1\nY 2\nZ 3")
    assert c[::-1] == deltakit_stim.Circuit("Z 3\nY 2\nX 1")
...
def test_with_inlined_feedback_bad_end_eats_into_loop():
    assert deltakit_stim.Circuit("""
        ...
        DETECTOR rec[-1] rec[-2]
        OBSERVABLE_INCLUDE(0) rec[-1]
    """).with_inlined_feedback() == deltakit_stim.Circuit("""
        CX 0 1
        M 1
        OBSERVABLE_INCLUDE(0) rec[-1]
        CX 0 1
    ...
    assert after == deltakit_stim.Circuit("""
        R 0 1 2 3 4 5 6
...
def test_with_inlined_feedback():
    ...
    """)
    after = before.with_inlined_feedback()
    assert str(after) == str(deltakit_stim.Circuit("""
        R 0 1 2 3 4 5 6
...
def test_detslice_ops_diagram_no_ticks_does_not_hang():
    assert deltakit_stim.Circuit.generated("surface_code:rotated_memory_x", rounds=5, distance=5).diagram("detslice-svg") is not None


def test_num_ticks():
    assert deltakit_stim.Circuit().num_ticks == 0
    assert deltakit_stim.Circuit("TICK").num_ticks == 1
    assert deltakit_stim.Circuit("""
        TICK
        REPEAT 100 {
            TICK
...
def test_max_mix_depolarization_is_allowed_in_dem_conversion_without_args():
    assert deltakit_stim.Circuit("""
        ...
        DETECTOR rec[-1]
        DETECTOR rec[-2]
    """).detector_error_model(approximate_disjoint_errors=True) == deltakit_stim.DetectorErrorModel("""
        error(0.5) D0
        error(0.5) D0 D1
        error(0.5) D1
    """)
...
def test_detslice_filter_coords_flexibility():
    c = deltakit_stim.Circuit.generated("repetition_code:memory", distance=3, rounds=3)
    d1 = c.diagram("detslice", filter_coords=[deltakit_stim.DemTarget.relative_detector_id(1)])
    d2 = c.diagram("detslice-svg", filter_coords=deltakit_stim.DemTarget.relative_detector_id(1))
    ...
    d1 = c.diagram("detslice", filter_coords=[deltakit_stim.DemTarget.relative_detector_id(1), deltakit_stim.DemTarget.relative_detector_id(3), deltakit_stim.DemTarget.relative_detector_id(5), "D7"])
...
def test_has_flow_ry():
    c = deltakit_stim.Circuit("""
        RY 0
    """)
    assert c.has_flow(deltakit_stim.Flow("1 -> Y"))
    assert not c.has_flow(deltakit_stim.Flow("1 -> -Y"))
    assert not c.has_flow(deltakit_stim.Flow("1 -> X"))
    assert c.has_flow(deltakit_stim.Flow("1 -> Y"), unsigned=True)
    assert not c.has_flow(deltakit_stim.Flow("1 -> X"), unsigned=True)
    assert c.has_flow(deltakit_stim.Flow("1 -> -Y"), unsigned=True)
...
def test_has_flow_cxs():
    ...
    assert c.has_flow(deltakit_stim.Flow("X_ -> YX"))
    assert c.has_flow(deltakit_stim.Flow("Y_ -> -XX"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> -XX"))

    assert c.has_flow(deltakit_stim.Flow("X_ -> YX"), unsigned=True)
    assert c.has_flow(deltakit_stim.Flow("Y_ -> -XX"), unsigned=True)
    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"), unsigned=True)
    assert not c.has_flow(deltakit_stim.Flow("X_ -> -XX"), unsigned=True)
...
def test_has_flow_cxm():
    c = deltakit_stim.Circuit("""
        ...
        M 1
    """)
    assert c.has_flow(deltakit_stim.Flow("1 -> _Z xor rec[0]"))
    assert c.has_flow(deltakit_stim.Flow("ZZ -> rec[0]"))
    assert c.has_flow(deltakit_stim.Flow("ZZ -> _Z"))
    assert c.has_flow(deltakit_stim.Flow("XX -> X_"))
    assert c.has_flow(deltakit_stim.Flow("1 -> _Z xor rec[0]"), unsigned=True)
    assert c.has_flow(deltakit_stim.Flow("ZZ -> rec[0]"), unsigned=True)
    assert c.has_flow(deltakit_stim.Flow("ZZ -> _Z"), unsigned=True)
    assert c.has_flow(deltakit_stim.Flow("XX -> X_"), unsigned=True)
...
def test_has_flow_lattice_surgery():
    ...
    """)
    assert c.has_flow(deltakit_stim.Flow("X_ -> YX"))
    assert c.has_flow(deltakit_stim.Flow("Z_ -> Z_"))
    assert c.has_flow(deltakit_stim.Flow("_X -> _X"))
    assert c.has_flow(deltakit_stim.Flow("_Z -> ZZ"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"))

    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> -YX"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"), unsigned=True)
    assert c.has_flow(deltakit_stim.Flow("X_ -> -YX"), unsigned=True)
...
def test_has_flow_lattice_surgery_without_feedback():
    c = deltakit_stim.Circuit("""
        ...
        S 0
    """)
    assert c.has_flow(deltakit_stim.Flow("X_ -> YX xor rec[1]"))
    assert c.has_flow(deltakit_stim.Flow("Z_ -> Z_"))
    assert c.has_flow(deltakit_stim.Flow("_X -> _X"))
    assert c.has_flow(deltakit_stim.Flow("_Z -> ZZ xor rec[0] xor rec[2]"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"))
    assert c.has_all_flows([])
    assert c.has_all_flows([
        deltakit_stim.Flow("X_ -> YX xor rec[1]"),
        deltakit_stim.Flow("Z_ -> Z_"),
    ])
    assert not c.has_all_flows([
        deltakit_stim.Flow("X_ -> YX xor rec[1]"),
        deltakit_stim.Flow("Z_ -> Z_"),
        deltakit_stim.Flow("X_ -> XX"),
    ])

    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> -YX"))
    assert not c.has_flow(deltakit_stim.Flow("X_ -> XX"), unsigned=True)
    assert c.has_flow(deltakit_stim.Flow("X_ -> -YX xor rec[1]"), unsigned=True)
...
def test_has_flow_shorthands():
    ...
    assert c.has_flow(deltakit_stim.Flow("X_ -> XX xor rec[1] xor rec[3]"))
    assert c.has_flow(deltakit_stim.Flow("Z_ -> Z_"))
    assert c.has_flow(deltakit_stim.Flow("_X -> _X"))
    assert c.has_flow(deltakit_stim.Flow("_Z -> ZZ xor rec[0] xor rec[2]"))

    assert c.has_flow(deltakit_stim.Flow("X_ -> XX xor rec[1] xor rec[3]"))
    assert not c.has_flow(deltakit_stim.Flow("Z_ -> -Z_"))
    assert not c.has_flow(deltakit_stim.Flow("-Z_ -> Z_"))
    assert not c.has_flow(deltakit_stim.Flow("Z_ -> X_"))
    assert c.has_flow(deltakit_stim.Flow("iX_ -> iXX xor rec[1] xor rec[3]"))
    assert not c.has_flow(deltakit_stim.Flow("-iX_ -> iXX xor rec[1] xor rec[3]"))
    assert c.has_flow(deltakit_stim.Flow("-iX_ -> -iXX xor rec[1] xor rec[3]"))
    with pytest.raises(ValueError, match="Anti-Hermitian"):
        deltakit_stim.Flow("iX_ -> XX")
...
def test_decomposed():
    assert deltakit_stim.Circuit("""
        ...
        TICK
        MPP X1*Z2*Y3
    """).decomposed() == deltakit_stim.Circuit("""
        H 0
        CX 0 1 1 0
        H 1
        S 1 0
...
def test_detecting_regions():
    assert deltakit_stim.Circuit('''
        ...
        MX 0 1
        DETECTOR rec[-1] rec[-2]
    ''').detecting_regions() == {deltakit_stim.DemTarget.relative_detector_id(0): {
        0: deltakit_stim.PauliString("Z_"),
        1: deltakit_stim.PauliString("X_"),
        2: deltakit_stim.PauliString("XX"),
    }}
...
def test_detecting_region_filters():
    ...
    assert len(c.detecting_regions(targets=[deltakit_stim.target_relative_detector_id(0), "D0"])) == 1
...
def test_detecting_regions_mzz():
    ...
    assert c.detecting_regions() == {
        deltakit_stim.target_relative_detector_id(0): {
            0: deltakit_stim.PauliString("__Z"),
            1: deltakit_stim.PauliString("__Z"),
        },
    }
...
def test_insert():
    ...
    with pytest.raises(ValueError, match='index <'):
        c.insert(1, deltakit_stim.CircuitInstruction("H", [1]))
    with pytest.raises(ValueError, match='index <'):
        c.insert(-1, deltakit_stim.CircuitInstruction("H", [1]))
    c.insert(0, deltakit_stim.CircuitInstruction("H", [1]))
    assert c == deltakit_stim.Circuit("""
        H 1
    """)

    with pytest.raises(ValueError, match='index <'):
        c.insert(2, deltakit_stim.CircuitInstruction("S", [2]))
    with pytest.raises(ValueError, match='index <'):
        c.insert(-2, deltakit_stim.CircuitInstruction("S", [2]))
    c.insert(0, deltakit_stim.CircuitInstruction("S", [2, 3]))
    assert c == deltakit_stim.Circuit("""
        S 2 3
        H 1
    """)

    c.insert(-1, deltakit_stim.Circuit("H 5\nM 2"))
    assert c == deltakit_stim.Circuit("""
        S 2 3
        H 5
        M 2
    ...
    c.insert(2, deltakit_stim.Circuit("""
        REPEAT 100 {
            M 3
        }
    """))
...
def test_pop():
    ...
    with pytest.raises(IndexError, match='index'):
        deltakit_stim.Circuit().pop(-1)
    with pytest.raises(IndexError, match='index'):
        deltakit_stim.Circuit().pop(0)
    c = deltakit_stim.Circuit("H 0")
    with pytest.raises(IndexError, match='index'):
        c.pop(1)
    with pytest.raises(IndexError, match='index'):
        c.pop(-2)
    assert c.pop(0) == deltakit_stim.CircuitInstruction("H", [0])
    c = deltakit_stim.Circuit("H 0\n X 1")
    assert c.pop() == deltakit_stim.CircuitInstruction("X", [1])
    assert c.pop() == deltakit_stim.CircuitInstruction("H", [0])


def test_circuit_create_with_odd_cx():
    with pytest.raises(ValueError, match="0, 1, 2"):
        deltakit_stim.Circuit("CX 0 1 2")


def test_to_tableau():
    assert deltakit_stim.Circuit().to_tableau() == deltakit_stim.Tableau(0)
    assert deltakit_stim.Circuit("QUBIT_COORDS 0").to_tableau() == deltakit_stim.Tableau(1)
    assert deltakit_stim.Circuit("I 0").to_tableau() == deltakit_stim.Tableau(1)
    assert deltakit_stim.Circuit("H 0").to_tableau() == deltakit_stim.Tableau.from_named_gate("H")
    assert deltakit_stim.Circuit("CX 0 1").to_tableau() == deltakit_stim.Tableau.from_named_gate("CX")
    assert deltakit_stim.Circuit("SPP Z0").to_tableau() == deltakit_stim.Tableau.from_named_gate("S")
    assert deltakit_stim.Circuit("SPP X0").to_tableau() == deltakit_stim.Tableau.from_named_gate("SQRT_X")
    assert deltakit_stim.Circuit("SPP_DAG Y0*Y1").to_tableau() == deltakit_stim.Tableau.from_named_gate("SQRT_YY_DAG")
...
def test_circuit_tags():
    ...
    assert c[0].tag == 'test'
    c.append(deltakit_stim.CircuitInstruction('CX', [0, 1], tag='test2'))
...
def test_circuit_eq_tags():
    assert deltakit_stim.CircuitInstruction("TICK", tag="a") == deltakit_stim.CircuitInstruction("TICK", tag="a")
    assert deltakit_stim.CircuitInstruction("TICK", tag="a") != deltakit_stim.CircuitInstruction("TICK", tag="b")
    assert deltakit_stim.CircuitRepeatBlock(1, deltakit_stim.Circuit(), tag="a") == deltakit_stim.CircuitRepeatBlock(1, deltakit_stim.Circuit(), tag="a")
    assert deltakit_stim.CircuitRepeatBlock(1, deltakit_stim.Circuit(), tag="a") != deltakit_stim.CircuitRepeatBlock(1, deltakit_stim.Circuit(), tag="b")
    assert deltakit_stim.Circuit("""
        H[test] 0
    """) == deltakit_stim.Circuit("""
        H[test] 0
...
def test_circuit_get_item_tags():
    ...
    """)[1] == deltakit_stim.CircuitInstruction("CX[test2] 1 2")
    assert deltakit_stim.Circuit("""
        H[test] 0
        CX[test2] 1 2
        REPEAT[test3] 3 {
    ...
    """)[2] == deltakit_stim.CircuitRepeatBlock(3, deltakit_stim.Circuit("M[test4](0.25) 4"), tag="test3")
    assert deltakit_stim.Circuit("""
        H[test] 0
        CX[test2] 1 2
        REPEAT[test3] 3 {
...
def test_tags_append():
    c = deltakit_stim.Circuit("""
        ...
        CX[test2] 1 2
    """)
    c.append(deltakit_stim.CircuitRepeatBlock(3, deltakit_stim.Circuit("""
        M[test4](0.25) 4
    """), tag="test3"))
    assert c == deltakit_stim.Circuit("""
        H[test] 0
...
def test_tag_approx_equals():
    assert not deltakit_stim.Circuit("H[test] 0").approx_equals(deltakit_stim.Circuit("H[test2] 0"), atol=3)
    assert deltakit_stim.Circuit("H[test] 0").approx_equals(deltakit_stim.Circuit("H[test] 0"), atol=3)


def test_tag_clear():
    c = deltakit_stim.Circuit("H[test] 0")
    c.clear()
    assert c == deltakit_stim.Circuit()
...
def test_tag_detecting_regions():
    assert deltakit_stim.Circuit("""
        ...
        M[test3](0.25) 0
        DETECTOR[test4](1, 2) rec[-1]
    """).detecting_regions() == {deltakit_stim.DemTarget('D0'): {0: deltakit_stim.PauliString("Z")}}


def test_tag_diagram():
    # TODO: include tags in diagrams
    assert str(deltakit_stim.Circuit("""
        R[test1] 0
        X_ERROR[test2](0.25) 0
        M[test3](0.25) 0
        DETECTOR[test4](1, 2) rec[-1]
...
def test_tag_from_file():
    c = deltakit_stim.Circuit.from_file(io.StringIO("""
        R[test1] 0
        REPEAT[test1.5] 2 {
            H[test2] 0
        }
...
def test_tag_insert():
    c = deltakit_stim.Circuit("""
        ...
        S[test2] 0
    """)
    c.insert(1, deltakit_stim.CircuitInstruction("CX[test3] 0 1"))
    assert c == deltakit_stim.Circuit("""
        H[test1] 0
        CX[test3] 0 1
        S[test2] 0
...
def test_append_tag():
    ...
    assert c == deltakit_stim.Circuit("H[test] 2 3")

    with pytest.raises(ValueError, match="tag"):
        c.append(c[0], tag="newtag")

    with pytest.raises(ValueError, match="tag"):
        c.append(deltakit_stim.CircuitRepeatBlock(10, deltakit_stim.Circuit()), tag="newtag")
...
def test_append_pauli_string():
    c = deltakit_stim.Circuit()
    c.append("MPP", [
        deltakit_stim.PauliString("X1*Y2*Z3"),
        deltakit_stim.target_y(4),
        deltakit_stim.PauliString("Z5"),
    ])
    assert c == deltakit_stim.Circuit("""
        MPP X1*Y2*Z3 Y4 Z5
    """)
    c.append("MPP", deltakit_stim.PauliString("X1*X2"))
    assert c == deltakit_stim.Circuit("""
...
def test_without_tags():
    circuit = deltakit_stim.Circuit("""
        H[tag] 5
    """)
    assert circuit.without_tags() == deltakit_stim.Circuit("""
        H 5
    """)


def test_reference_detector_and_observable_signs():
    det, obs = deltakit_stim.Circuit("""
        X 1
        M 0 1
        DETECTOR rec[-1]
        DETECTOR rec[-2]
    ...
    circuit = deltakit_stim.Circuit.generated("surface_code:rotated_memory_x", rounds=3, distance=3)
...
def test_append_circuit_to_circuit():
    circuit = deltakit_stim.Circuit("""
        H 0
    """)
    circuit.append(deltakit_stim.Circuit("""
        X 1
        Z 2
    """))
    assert circuit == deltakit_stim.Circuit("""
        H 0
        X 1
        Z 2
    """)
